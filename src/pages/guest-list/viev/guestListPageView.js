...
const GuestListPage = () => {
  ...
  useEffect(() => {
      async function fetchData() {
          await axios({
              method: 'GET',
              url: `${REACT_APP_BASE_URL}${REACT_APP_BOOKING_API}/bookings/requesters?page=${page}&size=${perPage}`
          })
              .then(async response => {
  ...
  const handlePerRowsChange = async (newPerPage, page) => {
      const response = await axios.get(`${REACT_APP_BASE_URL}${REACT_APP_BOOKING_API}/bookings/requesters?page=${page - 1}&size=${newPerPage}`);
      setData(response.data?.content.map((is, index) => {
          return (
              {
                  id: is.id,
